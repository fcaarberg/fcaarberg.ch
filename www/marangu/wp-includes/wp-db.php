...
<p>Das bedeutet entweder, dass Benutzername oder Passwort in der Datei <code>wp-config.php</code> falsch sind, oder der Datenbankserver <code>$dbhost</code> nicht erreichbar ist.</p>
<ul>
	<li>Bist Du sicher, da&szlig; Benutzername und Passwort stimmt?</li>
	<li>Bist Du sicher, da&szlig; der Name des Datenbankservers stimmt?</li>
	<li>Bist Du sicher, da&szlig; der Datenbankserver l&auml;uft?</li>
</ul>
<p>Wenn Du nicht sicher bist woran es liegt, kontaktiere am besten Deinen Hoster. Wenn Du weitergehende Hilfe brauchst, kannst Du Dich jederzeit an das <a href='http://wordpress-deutschland.org/'>Support-Forum</a> (<a href='http://wordpress.org/support/'>eng.</a>) wenden.</p>
");
			return;
		}
	...
	function select($db) {
		if (!@mysql_select_db($db, $this->dbh)) {
			$this->ready = false;
			$this->bail("
<h1>Datenbank kann nicht gew&auml;hlt werden</h1>
<p>Wir k&ouml;nnen zwar eine Verbindung zum Datenbankserver herstellen (das bedeutet, dass Dein Benutzername und das Passwort stimmen), aber die entsprechende Datenbank <code>$db</code> kann nicht gew&auml;hlt werden.</p>

<ul>
<li>Bist Du sicher, dass sie existiert?</li>
<li>Hat der Benutzer <code>".DB_USER."</code> die entsprechenden Rechte die <code>$db</code> Datenbank auch zu nutzen?</li>
<li>Bei einigen Systemen ist der Benutzername das Pr&auml;fix der Datenbank, k&ouml;nnte dass das Problem sein?</li>
</ul>
<p>Wenn Du nicht weisst, wie Du die Datenbank einrichtest, <strong>kontaktiere Deinen Provider</strong>. Wenn Du gar nicht weiterkommst, findest Du im <a href='http://wordpress.org/support/'>englischsprachigen-</a> und <a href='http://forum.wordpress-deutschland.org/'>deutschsprachigen-Supportforum</a> Hilfe.</p>");
			return;
		}
	}
